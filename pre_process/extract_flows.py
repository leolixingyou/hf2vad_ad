...
FLOWNET_INPUT_WIDTH = {"ped2": 512 * 2, "avenue": 512 * 2, "shanghaitech": 1024, "carla_local": 1024}
FLOWNET_INPUT_HEIGHT = {"ped2": 384 * 2, "avenue": 384 * 2, "shanghaitech": 640, "carla_local": 640}
...
def extracting_flows(dataset, dataset_name, of_save_dir):
    ...
    for idx, (batch, _) in tqdm(enumerate(dataset_loader), total=len(dataset)):
        ...
        # path to store flows
        image_dir = '/'.join(cur_img_addr.split('/')[-4:-1])
        video_of_path = os.path.join(of_save_dir, image_dir)
